...
import 'package:cabwire/presentation/passenger/passenger_history/ui/passenger_details_page.dart';
...
class PassengerHistoryPresenter extends BasePresenter<PassengerHistoryUiState> {
  ...
  PassengerHistoryUiState get currentUiState => uiState.value;
  ...
  void _setupFeedbackListener() {
    feedbackController.addListener(() {
      if (currentUiState.viewMode == PassengerHistoryViewMode.feedback) {
  ...
  void _handleExistingRideDetails(String rideId, bool showFeedbackForm) {
    if (showFeedbackForm) {
      ...
      uiState.value = currentUiState.copyWith(
        ...
        viewMode: PassengerHistoryViewMode.feedback,
        currentFeedbackText: '',
      );
    } else {
      ...
      uiState.value = currentUiState.copyWith(
        selectedRideId: rideId,
        viewMode: PassengerHistoryViewMode.details,
        currentFeedbackText: feedbackController.text,
      );
    }

    Get.to(() => PassengerRideDetailsScreen());
  }
